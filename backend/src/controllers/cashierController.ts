...
export const getOpenCashierSession = async (req: Request, res: Response): Promise<void> => {
...
export const openCashierSession = async (req: Request, res: Response): Promise<void> => {
  const { franchiseId } = req.user!;
  ...
  try {
    ...
    const sessionCode = `CS-${randomUUID().split('-')[0].toUpperCase()}`;
...
export const closeCashierSession = async (req: Request, res: Response): Promise<void> => {
  const { franchiseId } = req.user!;
  ...
  try {
    ...
    try {
      ...
      const parceledOrders = await dbClient.query(
...
export const getCashierHistory = async (req: Request, res: Response): Promise<void> => {
  const { franchiseId } = req.user!;
...
// Registrar sangria
export const registerSangria = async (req: Request, res: Response): Promise<void> => {
...
export const getSangrias = async (req: Request, res: Response): Promise<void> => {
...
export const getAllSangrias = async (req: Request, res: Response): Promise<void> => {
  const { franchiseId } = req.user!;
  if (!franchiseId) {
    res.status(400).json({ message: 'Usuário não está associado a uma franquia.' });
    return;
