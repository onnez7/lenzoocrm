...
export const getFranchiseChannel = async (req: Request, res: Response) => {
  try {
...
export const createFranchiseChannel = async (req: Request, res: Response) => {
  try {
    if (!req.user || !req.user.franchiseId) {
      return res.status(401).json({ message: 'Usuário não autenticado ou sem franquia.' });
    }
...
export const getMyFranchiseChannels = async (req: Request, res: Response) => {
  try {
    if (!req.user || !req.user.franchiseId) {
      return res.status(401).json({ message: 'Usuário não autenticado ou sem franquia.' });
    }
...
export const deleteFranchiseChannel = async (req: Request, res: Response) => {
  try {
    const channelId = parseInt(req.params.id);
    if (!req.user) {
      return res.status(401).json({ message: 'Usuário não autenticado.' });
