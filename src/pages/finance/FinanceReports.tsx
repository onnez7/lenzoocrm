...
const FinanceReports = () => {
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      case 'receivable': return <TrendingUp className="h-4 w-4 text-success" />;
      case 'payable': return <TrendingDown className="h-4 w-4 text-destructive" />;
      case 'salary': return <Users className="h-4 w-4 text-blue-600" />;
      case 'sangria': return <Wallet className="h-4 w-4 text-warning" />;
              ...
              {filteredHistory.length === 0 ? (
                ...
                filteredHistory.map((item) => (
                  <TableRow key={`${item.type}-${item.id}`}>
                    ...
                    <TableCell>
                      <span className={`font-medium ${item.amount >= 0 ? 'text-success' : 'text-destructive'}`}>
                    ...
                    <TableCell>
                      <Badge className={item.status === 'Pago' ? 'bg-success/10 text-success' : 'bg-warning/10 text-warning'}>
                        {item.status}
                      </Badge>
                    </TableCell>
                    <TableCell>
