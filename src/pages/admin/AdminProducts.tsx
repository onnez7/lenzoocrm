...
const AdminProducts = () => {
  ...
  const getStockStatus = (quantity: number, minStock: number) => {
    if (quantity <= 0) {
      return { badge: <Badge className="bg-red-100 text-red-800">Sem Estoque</Badge>, icon: <AlertTriangle className="h-4 w-4 text-destructive" /> };
    } else if (quantity <= minStock) {
      return { badge: <Badge className="bg-warning/10 text-warning">Estoque Baixo</Badge>, icon: <AlertTriangle className="h-4 w-4 text-yellow-600" /> };
    } else {
      return { badge: <Badge className="bg-success/10 text-success">Em Estoque</Badge>, icon: <CheckCircle className="h-4 w-4 text-success" /> };
    }
  };
