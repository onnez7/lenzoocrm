...
const CashierHistory = () => {
  ...
  const getStatusBadge = (status: string, difference: number | null) => {
    if (status === "open") {
      return <Badge variant="secondary" className="bg-success/10 text-success">Aberto</Badge>;
    }
    
    if (difference === 0) {
      return <Badge variant="secondary" className="bg-blue-100 text-blue-800">Conferido</Badge>;
    } else if (difference && difference > 0) {
      return <Badge variant="secondary" className="bg-warning/10 text-warning">Com Sobra</Badge>;
    } else {
                  ...
                  <TableCell>
                    {session.status === "closed" && session.difference !== null && (
                      <span className={`font-medium ${
                        ...
                        Number(session.difference) > 0 ? 'text-yellow-600' : 'text-destructive'
                      }`}>
