...
function getPaymentStatusBadge(status: string | undefined) {
  switch (status) {
    case "paid":
      return <Badge className="bg-success/10 text-success flex items-center gap-1"><CheckCircle className="h-3 w-3" /> Pago</Badge>;
    case "overdue":
      return <Badge className="bg-red-100 text-red-800 flex items-center gap-1"><AlertCircle className="h-3 w-3" /> Vencido</Badge>;
    case "pending":
      return <Badge className="bg-warning/10 text-warning flex items-center gap-1"><AlertCircle className="h-3 w-3" /> Em aberto</Badge>;
    default:
...
function getExtraBadges(opportunity: Opportunity) {
  ...
  if (opportunity.expected_close) {
    ...
    if (isBefore(closeDate, now)) {
      ...
      badges.push(<Badge key="soon" className="bg-warning/10 text-warning ml-1">Vencendo</Badge>);
    }
  }
