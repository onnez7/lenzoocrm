...
export function OpportunityManagement() {
  ...
  const getStageColor = (stage: string) => {
    switch (stage) {
      ...
      case 'Negociação': return 'bg-warning/10 text-warning';
      case 'Fechamento': return 'bg-success/10 text-success';
      default: return 'bg-secondary text-secondary-foreground';
    }
  };

  const getDaysInStageStatus = (days: number) => {
    if (days > 21) return { color: 'text-destructive', icon: AlertCircle };
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="p-4 border rounded-lg">
                  ...
                  <p className="text-2xl font-bold text-success">R$ 89K</p>
