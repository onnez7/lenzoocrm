...
async function createFreshTestRecords() {
  try {
    ...
    console.log('✅ Novos registros de teste criados com sucesso!');
    ...
    const newPayables = await pool.query('SELECT id, description, status FROM payables ORDER BY id');
    const newReceivables = await pool.query('SELECT id, description, status FROM receivables ORDER BY id');
    
    console.log('\n📋 IDs das contas a pagar criadas:');
    newPayables.rows.forEach(row => {
      console.log(`  - ID ${row.id}: ${row.description} (${row.status})`);
    });
    
    console.log('\n📋 IDs das contas a receber criadas:');
    newReceivables.rows.forEach(row => {
      console.log(`  - ID ${row.id}: ${row.description} (${row.status})`);
    });
